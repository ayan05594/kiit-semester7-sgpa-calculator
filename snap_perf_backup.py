...
class ScheduleFileParser:
    def __init__(self, schedule_file, filter_tags=None, filter_flag=None):
        ...
        for line in content.split('\n'):
            ...
            if line.startswith('[') and line.endswith(']'):
                if current_section:
                    ...
                    if self.filter_tags:
                        section_tag = current_params.get('tag', '').strip()
                        if section_tag in self.filter_tags or '*' in self.filter_tags:
                            self.filtered_sections.append(section_data)
                    elif self.filter_flag:
                        # For flag filtering, check if the flag matches any field using regex
                        section_flag = current_params.get('f', '').strip()
    ...
    def get_default_values(self):
        ...
        if not self.filtered_sections:
            if self.filter_tags:
                raise SnapPerfError(f"No job config found with tags '{self.filter_tags}' in schedule file {self.schedule_file}")
            elif self.filter_flag:
                raise SnapPerfError(f"No job config found with flag '{self.filter_flag}' in schedule file {self.schedule_file}")
            return {}
...
note:
  ...
  File naming convention: /home/(user)/suitename_compiler_perf_n/version_testname_tag.perf.data
            ''')
        ...
        # Mutually exclusive group for tags or flag filtering
        tag_group = optional.add_mutually_exclusive_group(required=True)
        ...
        optional.add_argument('-override', action='store_true',
                            help='Enable override mode for non-mandatory arguments. Use this flag when you need to override values from the existing schedule file for optional parameters.')
    ...
    def load_schedule_file(self):
        ...
        self.schedule_parser = ScheduleFileParser(self.args.ta, filter_tags, filter_flag)
        logging.info(f"Loaded schedule file: {self.args.ta}")
        logging.info(f"Found {len(self.schedule_parser.sections)} total sections")
        if filter_tags:
            logging.info(f"Found {self.schedule_parser.get_matching_configs_count()} sections matching tags '{filter_tags}'")
        elif filter_flag:
            logging.info(f"Found {self.schedule_parser.get_matching_configs_count()} sections matching flag '{filter_flag}'")
    ...
    def get_effective_values(self):
        ...
        for key in ['osversion', 'env', 'f']:
            ...
            if self.args.override and cmd_value is not None:
        ...
        # Handle tags or flag filter
        if hasattr(self.args, 'tags') and self.args.tags:
            effective['tag'] = ' '.join(self.args.tags) if isinstance(self.args.tags, list) else self.args.tags
    ...
    def get_next_directory_number(self, base_path, suite_compiler_prefix):
        ...
        # Pattern to match existing directories
        pattern = os.path.join(base_path, f"{suite_compiler_prefix}_perf_*")
        ...
        for dir_path in existing_dirs:
            ...
            match = re.search(r'_perf_(\d+)$', dir_name)
            if match:
    ...
    def get_output_directory_info(self):
        ...
        if self._dir_number is not None:
            ...
            suite_compiler_perf_dir = f"{suite_compiler_prefix}_perf_{self._dir_number}"
            return base_path, suite_compiler_prefix, self._dir_number, suite_compiler_perf_dir
    ...
    def show_individual_runtest_commands(self):
        ...
        for version in self.args.versions:
            for test in self.args.test:
                combinations.append((version, test))
        
        for i, (version, test) in enumerate(combinations, 1):
    ...
    def generate_individual_runtest_command(self, effective_values, version, test):
        ...
        make_cmd = self.generate_make_command(effective_values, version, test)
    ...
    def generate_schedule_content(self):
        ...
        # Generate one section per version (no test field) - runta will filter tests with --override
        logging.info(f"Generating {len(self.args.versions)} version sections: {self.args.versions}")
        
        for i, version in enumerate(self.args.versions):
            if i > 0:
                content += "\n\n"
            
            section_header = self.generate_section_header(effective_values, version, None)
            # Use placeholder make command - will be overridden per test execution
            # This prevents conflicts since we always override with specific make commands
            perf_dir = self.get_output_directory_info()[0]  # Get base path
            placeholder_perf_file = f"{perf_dir}/placeholder.perf.data"
            make_command = f'RUN="taskset 0x2 perf record -o {placeholder_perf_file}"'
            ...
            content += f"compiler : {effective_values.get('compiler', '')}\n"
            content += f"user : {effective_values['user']}\n"
            content += f"suite : {effective_values['suite']}\n"
            
            if 'jobs' in self.schedule_parser.get_default_values():
                content += f"jobs : {self.schedule_parser.get_default_values()['jobs']}\n"
            
            content += f"osversion : {effective_values.get('osversion', '')}\n"
            
            if effective_values.get('env'):
                env_value = effective_values['env'].strip("'\"")  # Remove any existing quotes
                content += f"env : '{env_value}'\n"
            
            if effective_values.get('f'):
                f_value = effective_values['f'].strip("'\"")  # Remove any existing quotes
                content += f"f : '{f_value}'\n"
            
            content += f"make : '{make_command}'\n"
            content += f"version : {version}\n"
            
            if effective_values.get('tag'):
                content += f"tag : {effective_values['tag']}\n"
    ...
    def write_schedule_file(self, content):
        # Schedule file should be saved in the same directory as perf data
        if self.args.path:
            base_output_path = self.args.path
        else:
            base_output_path = os.path.expanduser("~")
        ...
        base_output_path, suite_compiler_prefix, dir_number, suite_compiler_perf_dir = self.get_output_directory_info()
        output_dir = os.path.join(base_output_path, suite_compiler_perf_dir)
        ...
        # Required naming convention: suitename_compiler_perf.cfg
        effective_values = self.get_effective_values()
        safe_compiler = effective_values['compiler'].replace('/', '_').replace(' ', '_')
        filename = f"{effective_values['suite']}_{safe_compiler}_perf.cfg"
        self.output_file = os.path.join(output_dir, filename)
        
        with open(self.output_file, 'w') as f:
            f.write(content)
        
        combinations_count = len(self.args.versions) * len(self.args.test)
        logging.info(f"Generated schedule file with {combinations_count} combinations: {self.output_file}")
        return self.output_file
    
    def generate_runtest_script(self):
        """Generate runtest.sh script in the same directory as perf data"""
        # Get the directory where perf data will be saved
        if self.args.path:
            base_output_path = self.args.path
        else:
            base_output_path = os.path.expanduser("~")
        ...
        with open(runtest_script_path, 'w') as f:
            ...
            # Generate individual version-test combination commands
            effective_values = self.get_effective_values()
            combo_num = 1
            for version in self.args.versions:
                for test in self.args.test:
                    test_make_cmd = self.generate_make_command(effective_values, version, test)
                    ...
                    # Add test, version and make overrides
                    individual_cmd.extend(['--override', f'test={test}', f'version={version}', f'make={test_make_cmd}'])
                    
                    f.write(f'# Runta command {combo_num}: version={version}, test={test}\n')
            ...
            # Generate the equivalent runtest commands for all combinations
            combo_num = 1
            for version in self.args.versions:
                for test in self.args.test:
                    runtest_cmd = self.generate_individual_runtest_command(effective_values, version, test)
                    f.write(f'# Runtest command {combo_num}: version={version}, test={test}\n')
                    f.write(f'{runtest_cmd}\n')
                    f.write(f'\n')
                    combo_num += 1
    ...
    def run_runta(self):
        ...
        # Execute runta for each version-test combination using --override
        effective_values = self.get_effective_values()
        all_success = True
        for version in self.args.versions:
            for test in self.args.test:
                ...
                # Add test and version override with make command override for this specific combination
                test_make_cmd = self.generate_make_command(effective_values, version, test)
                cmd.extend(['--override', f'test={test}', f'version={version}', f'make={test_make_cmd}'])
                ...
                if self.args.dryrun:
                    logging.info(f"DRY RUN - Would execute runta for version {version}, test {test}: {' '.join(cmd)}")
                    # Still execute runta with --dryrun flag to see what it would do
                
                logging.info(f"Executing runta for version {version}, test {test}: {' '.join(cmd)}")
                ...
                try:
                    ...
                    if return_code == 0:
                        logging.info(f"Runta execution completed successfully for version {version}, test {test}")
                    else:
                        logging.error(f"Runta execution failed for version {version}, test {test} with return code {return_code}")
                        all_success = False
                        
                except Exception as e:
                    logging.error(f"Runta execution failed for version {version}, test {test}: {e}")
                    all_success = False
    ...
    def run(self):
        try:
            ...
            combinations_count = len(self.args.versions) * len(self.args.test)
            logging.info(f"Processing {len(self.args.versions)} versions and {len(self.args.test)} tests")
            logging.info(f"Total combinations to generate: {combinations_count}")
            logging.info(f"Versions: {', '.join(self.args.versions)}")
            logging.info(f"Tests: {', '.join(self.args.test)}")
            
            # Show output path structure according to architectural requirements
            base_path, suite_compiler_prefix, dir_number, suite_compiler_perf_dir = self.get_output_directory_info()
            logging.info(f"Output structure: {base_path}/{suite_compiler_perf_dir}/{{version}}_{{testname}}_{{tag}}.perf.data")
            logging.info(f"Versions: {', '.join(self.args.versions)}")
            logging.info(f"Tests: {', '.join(self.args.test)}")
            if hasattr(self.args, 'tags') and self.args.tags:
            ...
            logging.info(f"Profiling session completed successfully for {combinations_count} combinations")
