...
note:
  ...
  Long version/compiler strings are automatically truncated to prevent database warnings.
    ...
    def get_effective_values_for_section(self, section):
        ...
        effective['suite'] = self.args.suite
        effective['utility'] = self.args.utility
        # Handle tags or flag filter - use the tag from this specific section
        if hasattr(self.args, 'tags') and self.args.tags:
            effective['tag'] = ' '.join(self.args.tags) if isinstance(self.args.tags, list) else self.args.tags
        elif hasattr(self.args, 'flag_filter') and self.args.flag_filter:
    ...
    def generate_schedule_content(self):
        ...
        # Get all filtered sections that match the tag/flag criteria
        ...
        # Use the FIRST matching section as a template for all versions
        # This ensures we generate exactly m job configs for m versions
        template_section = filtered_sections[0]
        logging.info(f"Using template section with tag '{template_section['params'].get('tag', '')}' for {len(self.args.versions)} versions")
        logging.info(f"Generating {len(self.args.versions)} job configs (one per version)")
        logging.info(f"Versions: {self.args.versions}")
        
        # Generate one section per version using the template section
        for i, version in enumerate(self.args.versions):
            if i > 0:
                content += "\n\n"
            
            # Create effective values for this version using the template section
            effective_values = self.get_effective_values_for_section(template_section)
            
            section_header = self.generate_section_header_for_section(effective_values, version, template_section)
            ...
            # Create make command with version-specific perf file name
            # The test name will be substituted by runta when it processes each test
            perf_filename_template = f"{safe_version}_$(TEST)_{safe_tag}.perf.data"
            perf_output = os.path.join(perf_dir, perf_filename_template)
            ...
            db_safe_compiler = self.truncate_for_database(effective_values.get('compiler', ''), 45)
            content += f"compiler : {db_safe_compiler}\n"
            content += f"user : {effective_values['user']}\n"
            content += f"suite : {effective_values['suite']}\n"
            
            if 'jobs' in template_section['params']:
                content += f"jobs : {template_section['params']['jobs']}\n"
            ...
            content += f"version : {db_safe_version}\n"
    ...
    def write_schedule_file(self, content):
        ...
        # Calculate the correct count: m versions = m job configs, n tests × m configs = total perf files
        job_configs_count = len(self.args.versions)
        ...
        logging.info(f"Each test will generate {job_configs_count} perf files (one per version)")
        ...
        with open(runtest_script_path, 'w') as f:
            ...
            for test in self.args.test:
                ...
                for i, version in enumerate(self.args.versions):
                    safe_version = version.replace('/', '_').replace(' ', '_')
                    safe_tag = effective_values.get('tag', '').replace('/', '_').replace(' ', '_')
                    perf_file = f"{safe_version}_{test}_{safe_tag}.perf.data"
                    if i > 0:
                        f.write(', ')
                    f.write(perf_file)
                f.write('\n')
    ...
    def run(self):
        try:
            ...
            job_configs_count = len(self.args.versions)  # m versions = m job configs
            total_perf_files = job_configs_count * len(self.args.test)  # m configs × n tests = total perf files
            logging.info(f"Processing {len(self.args.versions)} versions, {filtered_sections_count} matching sections (using first as template), and {len(self.args.test)} tests")
            logging.info(f"Will generate {job_configs_count} job configs in schedule file")
            logging.info(f"Will execute {len(self.args.test)} runta commands (one per test)")
            logging.info(f"Total perf files to generate: {total_perf_files}")
