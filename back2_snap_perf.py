...
examples:
  ...
  proj/ta/bin/snap_perf -compiler grco-nightly-gcc -versions install -suite TSVC_2_Single_Kernel_Binary -test s000 -ta grco-gcc-perf.cfg -user grco -custom "gcc=$LHOME/gcc-trunk version=install"
...
note:
  ...
  Long version/compiler strings are automatically truncated to prevent database warnings.
        ...
        optional.add_argument('-custom',
                            help='Custom compiler configuration passed to runta --override (e.g., "gcc=$LHOME/gcc-trunk version=install"). All custom parameters are combined with test override into a single --override argument.')
    ...
    def generate_runtest_script(self):
        ...
        with open(runtest_script_path, 'w') as f:
            ...
            # Generate and show all individual runtest commands that runta will execute
            cmd_counter = 1
            for test in self.args.test:
                # Get all filtered sections that match the suite and tag/flag criteria
                filtered_sections = self.schedule_parser.get_filtered_sections()
                ...
                for section in filtered_sections:
                    for version in self.args.versions:
                        ...
                        effective_values = self.get_effective_values_for_section(section)
                        runtest_cmd = self.generate_individual_runtest_command(effective_values, version, test)
                        f.write(f'# {cmd_counter}. {runtest_cmd}\n')
                        cmd_counter += 1
                f.write(f'\n')
            
            # Calculate correct totals: tests × configs (number of sections in schedule file)
            filtered_sections = self.schedule_parser.get_filtered_sections()
            num_configs = len(filtered_sections) * len(self.args.versions)  # sections × versions = configs in schedule file
